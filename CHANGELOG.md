...
- **同期させるものが無い.** 命令が名乗るのは**種類**であって実物ではない —
  ...
  行の指定を外へ出すのは **Me Dock の「[Format] Me」メニュー**(v4.0.162。それ以前はパレットのみ)。
  同じメニューの**チェック**を入れると、以後は書式ボタン(ハイライト/取消線/太字/斜体)が
  **最初からFC形で書く**(`□ Folding Comment(FC) below`)。外す/戻すはいつでもできるso、**どちらで書くかは自由**。
