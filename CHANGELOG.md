...
- **同期させるものが無い.** 指定は自分の相手を名乗る（`A↑2{150%(白/緑)}`）。同じものが並んだら**出現順**で結ぶので、
  ファイルは**腐りうるラベルも番号も持たない**。2つめを指したい時だけ `A↑2#2{…}` と書く。
  行の指定を外へ出すコマンドはパレットにある。
- **上付きが散文に不意打ちしない.** `x↑2` や `(a+b)↑2` は上付きになるが、`、↑2` や `🐱↑3` は**ただの字**のまま。
  上付きは「載る相手」があってのものだから。それでも上付きにしたければ、コメントで名乗る —
  `🐱↑3<!-- Mew! 🐱↑3{150%(白/緑)} -->`。MeOSの他と同じ規則（**素はただの字・命令はコメントに書く**）なので、
  **エスケープ文字は要らない**。
