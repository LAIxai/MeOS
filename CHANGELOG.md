...
### v4.2.59 (2026-09-11)
- **MeOS puts out its own fires before it leaves.** Install a new version and the editor shuts the old copy down and loads the new one in the same process — but shutting down only cleared decorations, never the repeating beats a running clock had started. Each install could leave one beating. They are all cleared on the way out now. (This was written as the cause of a bell that would not stop; it was not — see v4.2.60. It is still worth doing.)
...
- **The bar says what the next clock is.** After three minutes it reads `⏰ 目薬の時間です — click to start 1m timer`, and it blinks yellow while it waits, so you can see from across the room that a click is owed.
- **Message lines belong to the membrane, not to the clock above them.** A line that never folds (UFC) breaks a run of comment lines in two — that is how it stays in view — but it was also being taken as the *owner* of everything below it. So the messages under a second clock opened only when the caret sat on that clock's own line, not when it was on the membrane. The fold still stops at the same place; only the question of who owns the lines has changed.

### v4.2.53 (2026-09-11)
- **A clock with no starting time is no longer given one.** Since v4.2.33 MeOS wrote the moment such a clock began into its line. Around a loop that was fatal: on the second turn the written moment was already past, its rounds long spent, and the clock was read as finished before it could run. Having no starting time is what that clock *is* — it begins when you click — so writing one erased the very thing that made it work. The moment is now held in memory alone and let go when the turn passes.
- **A starting point that has gone by now says so.** Read the raw line `2026-09-11 00:54` and nothing tells you whether that time is still ahead or long behind — you would have to check the clock. Written as `00:54p` it reads in one character. The letter is added when the clock is armed, once, with nothing swept.
- **A pair whose goal has passed is folded back into one.** In `2027-09-09 15:30f/2026-09-10 09:29p` the **p** is only there to keep the stopwatch honest across a restart while the goal is still ahead. Once the goal is behind, that job is done: what remains is a starting point, and it is written `2027-09-09 15:30p`.
...
- *A backtick inside a CSS comment would have turned the whole of Me Dock into a string; `check_webview.js` caught it before it shipped.*
