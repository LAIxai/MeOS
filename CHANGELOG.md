...
- **命令が本当のこと.** コメントの `-1.` を `-` に書き換えると、行頭の `1. ` を MeOS が `- ` に直す（逆も同じ）。
  書き手が触るのは、コメントの中の一文字だけでいい。
- **広い表をペインに収める.** 収まらない表は、はみ出している**列ごと**に小さく描いて桁を揃えたまま収める。
  生データは1文字も変えない。読みたい行にカーソルを置けば原寸に戻る。`MeOS: Fit Tables to Width…` で目標桁数を選ぶ。
