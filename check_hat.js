...
console.log('㉜ v4.0.319 チェックの時刻は「塊を離れた時」に入れる（打鍵ごとでも、保存待ちでもなく）');
{
  let _v = 0;
  const mk = (L) => ({ lineCount: L.length, isClosed: false, version: ++_v, uri: { toString: () => 'file:///w', scheme: 'file' }, eol: 1,
    getText: () => L.join('\n'), lineAt: (n) => ({ text: L[n] || '' }) });
  const L = ['## 見出しテスト', '<!-- Mew!FC H2_2026.08.21(F)am01:21.49JST (白/green)//[す]tip= -->', '', 'ふつうの文'];
  const doc = mk(L);
  const p = T.meosCheckStampPendingAt(doc, 1);
  ok(!!p && p.line === 1, '時刻待ちの箱を見つける', p);
  ok(!!p && L[1].slice(p.ins - 1, p.ins + 1) === 'す]', '入れるのは中身の直後(閉じ括弧の手前)', p && L[1].slice(p.ins - 1, p.ins + 1));
  const r0 = T.meosCheckStampRange(doc, 0), r1 = T.meosCheckStampRange(doc, 1);
  ok(r0.from === r1.from && r0.to === r1.to, '本文行とFC行は同じ塊= 行き来しても確定しない', [r0, r1]);
  ok(T.meosCheckStampRange(doc, 3).from === 3, '離れた行は別の範囲= 出れば確定する', 3);
...
//    バッジを入れたFC膜を置くんだよ」）バッジの引越し。read-both＝旧形も読む・書くのは新形だけ。
{
  ...
  // ★v4.0.331（俊克 pm00:42 バグ1「Createボタンを押すと、FCバッジコメントが出てこない」）
  //   膜を作る道は4つ在る。**どの道から作っても**バッジ行が付くことを、道ごとに確かめる。
  const dm = { languageId: 'markdown' };
  ok(tpl.close.split('\n').length === 2 && T.meosIsPairBadgeSpec(tpl.close.split('\n')[1]),
     '道①②(Add Membrane / Create Me)＝ close が閉じ膜とバッジの2行を持つ', '2行');
  const cb = T.meosNewMembraneCloseBlock(dm, '', 'Y_1', '');
  ok(cb.split('\n').length === 2 && T.meosIsPairBadgeSpec(cb.split('\n')[1]),
     '道③④(新規.mdの最初の膜 / 表の膜化)＝ 同じ1つの関数から出る', '2行');
  ok(T.parseMstatBadgeFromText(T.buildMembraneOpenLine(dm, '', 'Y_1', 'x', '')) === null,
  ...
  // ★v4.0.335（俊克 pm05:51 改良1「膜の色がFC化によって正しく反映されてない。Mepyで膜色を変更しても
  //   Yが変わらない。膜の色も紫色のまま」）＝ 描画は▼行を見たままだった。
  const CLR = ['<!-- {* ▼mCN=G // 緑の膜 *} -->', '本文', '<!-- {* ▲mCN=G // *} -->',
               '<!-- Mew!FC mCN (📊⊕0+0D-2G) -->'];
  ok(T.meosMembraneColorCodeForOpen(mk(CLR), 0, null) === 'G',
     '★新形＝ 開始膜の行から引いても、▲の次のバッジの色が返る', 'G');
  ok(T.meosMembraneColorForOpen(mk(CLR), 0, null, 'FALLBACK') === T.membraneCssColorForCode('G'),
     '色が引けるので、深さの色（紫）に落ちない', '緑');
  const OLDC = ['<!-- {* ▼mCN=H // 旧形 (📊0⊕0+0D-2Y) *} -->', '本文', '<!-- {* ▲mCN=H // *} -->'];
  ok(T.meosMembraneColorCodeForOpen(mk(OLDC), 0, null) === 'Y',
  ...
  // ★v4.0.338（俊克 バグ1「見出しで、FC膜に文字カーソルがあるとき、見出しが生データで表示されなくなった」）
  //   ＝ 橙に染まる行と、生データを見せる行を別々の物差しで決めていた。同じ1つの判定から引く。
  const HD = ['## 見出し', '<!-- Mew!FC H2_x (白/red)//[]tip= -->', 'つぎの文'];
  const rawSet = (ln) => { const m = T.meosFcMate(mk(HD), ln);
    const s2 = new Set([ln]); if (m && m.b && m.b.open == null && m.mate >= 0) s2.add(m.mate);
    return [...s2].sort((a, b) => a - b).join(','); };
  ok(rawSet(1) === '0,1', '★FC行に居ると、見出しの行も生データになる（直している当の物が見える）', '0,1');
  ok(rawSet(0) === '0,1', '見出しに居ても同じ2行', '0,1');
  ok(rawSet(2) === '2', '関係ない行は自分だけ', '2');
  // 表は「カーソルの行と、その1枚」だけ＝ 他の行は正しいレンダリングのまま（v4.0.323の約束）
  const TB2 = ['| a |', '| b |', '| c |', '<!-- Mew!FC not -->', '<!-- Mew!FC == (白/黄) -->', '<!-- Mew!FC not -->'];
  const rs2 = (ln) => { const m = T.meosFcMate(mk(TB2), ln);
    const s2 = new Set([ln]); if (m && m.b && m.b.open == null && m.mate >= 0) s2.add(m.mate);
    return [...s2].sort((a, b) => a - b).join(','); };
  ok(rs2(4) === '1,4', '表は 対の2行だけ生データ（他の行は正しい姿のまま）', '1,4');

  // ★v4.0.345（俊克 バグ1「開始膜にカーソルがいるとき、閉じ膜が生データになってないよ」）
  //   ＝ 膜は ▼▲バッジの3行がひと組なので、どれに居ても3行とも生データになる。
  const MB = ['// {▼mCN=A // 作業ログ}', '本文', '// {▲mCN=A //}', '<!-- Mew!FC mCN (📊⊖9+1D-1W) -->'];
  const rs3 = (ln) => { const m = T.meosFcMate(mk(MB), ln);
    const s3 = new Set([ln]); if (m) for (const l of (m.lines || [])) if (l >= 0) s3.add(l);
    return [...s3].sort((a, b) => a - b).join(','); };
  ok(rs3(0) === '0,2,3', '★開始膜に居ると、閉じ膜もバッジも生データ', '0,2,3');
  ok(rs3(2) === '0,2,3', '閉じ膜に居ても同じ3行', '0,2,3');
  ok(rs3(3) === '0,2,3', 'バッジ行に居ても同じ3行', '0,2,3');
  ok(rs3(1) === '1', '膜の本文は巻き込まない', '1');

  // ★★v4.0.346（俊克 バグ1「見出しの修正が元に戻ってしまった。直してといった後、直してなかったのかな?」）
  //   ★★直っていなかった。v4.0.338で書き換えたのは「文字を伏せる下ごしらえ」の側で、
  //     本物の門番（装飾を描くかどうか）は別の6行が古い物差しのままだった。
  //   ★★そして ㉞ の検査が その嘘を通した ＝ 写像(meosFcMate)しか見ておらず、
  //     その答えを誰がどう使うかを1つも数えていなかった。
  //   → **使う側を数える検査**をここに置く。物差しが2本に割れたら、この1件が落ちる。
  {
    const src = fs.readFileSync(SRC, 'utf8');
    const strays = (src.match(/line [!=]== docCursorLine/g) || []).length;
    ok(strays === 0, '★生データの判定を通さない門番が1つも残っていない（物差しは1本）', '0件 / 実際 ' + strays + '件');
    const users = (src.match(/_isRawLine\(/g) || []).length;
    ok(users >= 10, '★判定を使っている所が十分ある（下ごしらえ側だけ替えて満足しない）', '10か所以上 / 実際 ' + users);
    // ★★v4.0.347（俊克「ハイライトが直ってない。上付き/下付きも直ってない。取消線だけ直っている」）
    //   ＝ 同じ判定が8か所に別々に書かれていた。取消線だけ直ったのは1本目だけ直したから。
    //   → **自前で数えている所が1つでも残っていたら落ちる**検査を置く。
    const own = (src.match(/cursorLines = new Set\(/g) || []).length;
    ok(own === 0, '★カーソル行を自前で数えている描画パスが1つも残っていない', '0件 / 実際 ' + own + '件');
    const asks = (src.match(/meosRawLines\(editor\)/g) || []).length;
    ok(asks >= 9, '★描画パスが全部その1つに訊いている', '9か所以上 / 実際 ' + asks);
    // ★計測の網＝ 描画パスに1つも計測が無かった(v4.0.112「網の外に居る証明」)
    const meters = (src.match(/\[(Bold|MeTex|MeLink|Func|TableMerge|TableCalc|TableRowLine|TableFit|ImageThumb)\] '/g) || []).length;
    ok(meters >= 9, '★描画パスに計測が入っている（網の外を作らない）', '9か所以上 / 実際 ' + meters);
  }
  // ★v4.0.345(俊克「もう元に戻す仕組みも要らないよ。退路を断つ」): 端の行き先を確かめていた検査(v4.0.338〜341)は
  //   仕掛けごと落とした= 膜の行も生データを見せるので、跨ぐ端が存在しない。
}
