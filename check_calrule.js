...
console.log('⑤ Date&Count(v4.2.347 ~日付 → ×N)');
const Y=new Date().getFullYear()+1;   // 来年にすれば、いつ走らせても未来
let u=X.meosParseUntil(Y+'.09.26(土)am11:21.07f');
ok(u && u.getFullYear()===Y && u.getMonth()===8 && u.getDate()===26 && u.getHours()===11 && u.getMinutes()===21 && u.getSeconds()===7, 'MeOSの日時の形(曜日・am・秒・f付き)を読む', u&&u.toString());
u=X.meosParseUntil(Y+'.09.26(s)pm03:05JST'); ok(u && u.getHours()===15 && u.getMinutes()===5, 'pm03:05 → 15:05・JSTは捨てる', u&&u.toString());
u=X.meosParseUntil(Y+'-10-31'); ok(u && u.getHours()===23 && u.getMinutes()===59, '日付だけ= その日いっぱい', u&&u.toString());
u=X.meosParseUntil(Y+'-10-31 10:00'); ok(u && u.getHours()===10, '普通の形', u&&u.toString());
ok(X.meosParseUntil('あいう')===null, '読めない字は null');
const rw=(line,now)=>{const c=X.meosClockFcParse(line);const o=X.meosClockOriginOf(c);const n=X.meosCycleRoundsUntil(o,c.cycle,c.untilAt);return {c,n,out:X.meosClockUntilRewrite(line,c,n)};};
let r=rw('<!-- Mew!UFC ⏰ '+Y+'-09-24 10:00 ↺8h ~'+Y+'-10-31 10:00 // 服薬 -->');
ok(r.c.cycle.join('|')==='8h' && !r.c.rounds, '~日付 は周期に混ざらない(2026を2026分と読まない)', r.c.cycle);
ok(r.n===111 && r.out==='<!-- Mew!UFC ⏰ '+Y+'-09-24 10:00 ↺8h ×111 // 服薬 -->', '↺8h ~10/31 10:00 → ×111(ちょうど)', r.out);
r=rw('<!-- Mew!UFC ⏰ '+Y+'-09-24 10:00 ↺8h ~'+Y+'-10-31 09:59 // 服薬 -->');
ok(r.n===110, '1分足りなければ ×110(越えない一番大きいN)', r.n);
r=rw('<!-- Mew!UFC ⏰ '+Y+'-10-07 10:00 ↺t1,3 ~'+Y+'-03-31 -->');
r=rw('<!-- Mew!UFC ⏰ 2027-10-07 10:00 ↺t1,3 ~2028-03-31 -->');
ok(r.out==='<!-- Mew!UFC ⏰ 2027-10-07 10:00 ↺t1,3 ×11 -->', '暦の規則: 2027/10/7〜2028/3/31 の第1第3木曜は12回 → ×11', [r.n,r.out]);
r=rw('<!-- Mew!UFC ⏰ '+Y+'-09-24 21:00 ↺↻3m 1m ~'+Y+'-09-24 21:13 // ボクシング -->');
ok(r.out==='<!-- Mew!UFC ⏰ '+Y+'-09-24 21:00 ↺↻(3m 1m)×3 // ボクシング -->', '並びは括弧で包む(3m 1m)×3', r.out);
r=rw('<!-- Mew!UFC ⏰ '+Y+'-09-24 21:00 ↺↻(3m 1m)×9 ~'+Y+'-09-24 21:13 -->');
ok(r.out==='<!-- Mew!UFC ⏰ '+Y+'-09-24 21:00 ↺↻(3m 1m)×3 -->', '既に×Nが在れば数を替える', r.out);
r=rw('<!-- Mew!UFC ⏰ '+Y+'-09-24 21:00 ↺↻((30s 15s)×4 1m) ~'+Y+'-09-24 21:12 -->');
ok(r.out==='<!-- Mew!UFC ⏰ '+Y+'-09-24 21:00 ↺↻((30s 15s)×4 1m)×3 -->', '入れ子: 外の括弧に ×3', r.out);
r=rw('<!-- Mew!UFC ⏰ 2. ↺(5m) ~'+Y+'-09-24 21:12 -->');
