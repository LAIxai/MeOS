// MeOS menu-bar helper (v4.2.364) — runs as a LaunchAgent via `osascript -l JavaScript`, so it lives on
...
function run(argv) {
  ...
  function step() {
    ...
    if (ownerAlive(owner)) {
      ...
      menu.push({ id: 'h:quithelper', title: 'Quit V-helper (' + appName + ') itself' });   // v4.2.364
