...
💓 an hour of boxing `((3m 1m)×5)×3 // 💓HIIT-Box 1. // 💓break`, one message for each time — in yellow, blue and green, and the caret lands on the last `×N` with a drum beside
...
<!-- Mew!UFC ⏰ 09:00 ↺5m -->
<!-- Mew!UFC ⏰🔐 2026-09-03 12:10 ↻15 -->
```

**How precisely you write it says what kind of plan it is.** A time alone means today — or tomorrow if it has
gone. A date means that day. Nothing has to be remembered about where you put it, because it is written where
it belongs, and it travels with the file.

**Add an arrow and it repeats — and the arrow says which way time runs.** `↺` turns anticlockwise for a
countdown: `↺5m` every five minutes, `↺8h` every eight hours, `↺30d` every thirty days, `↺50/10` alternating
fifty minutes and ten. `↻` turns clockwise for a **stopwatch**: `↻15` climbs from zero for fifteen minutes,
goes back to zero and measures again. Both ring at the same instants; only the figure is read the other way
round, and neither stops until you stop it. The time you write is the **origin** and never moves — every bell
is counted on from it, so nothing is stored twice and the day you started stays in the file.

**Setting one is a small panel under ⏰▾**: the origin (with its weekday, and it may be in the past for a
repeat), how long each turn lasts, which way it runs, and 🔐 if this one must not be droppable. Turn the drums,
...
⏰ also keeps **the next five clocks**, soonest first. A row is the membrane's name — choose it and you are
there, on the line the clock is written on, because anyone can set one by accident and finding it should not
be a search. The one the ⏰ button is counting keeps its figure beside it so you can see which row that number
belongs to. ☑ and ☐ let a clock rest without losing what it was set for, `Stop` in the last minute is a pause
...
- **A clock on a membrane (v4.1)** — write `<!-- Mew!UFC ⏰ 18:30 -->` under a membrane and MeOS brings you
  back to it at that time; `↺5m` returns every five minutes, `↻15` measures fifteen and starts again. The
  schedule lives in the place it is about, so there is nothing to remember about where you filed it, and it
  travels with the file. The countdown is called in stages, like an archery line.
- **Tag & Go (v4.1)** — put `#eyedrops` in a membrane's opening comment and it joins that tag's list, clock or
  no clock; choose a row to warp there, tick its box to start the clock written on it. The ⏰ is the face of it
  because a clock explains itself; what is underneath is a way back into your own writing.
