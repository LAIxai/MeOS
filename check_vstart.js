...
(async()=>{
 ...
 // ★v4.2.90(俊克 改良1「止めて、最初から始められるか」)= 止めた行から v を消して ▶️ → 最初から
 A=['# t','<!-- {* ▼mCN=R_1 // c *} -->','x','<!-- {* ▲mCN=R_1 // c *} -->','<!-- Mew!UFC ⏰ ↻5m ×1 -->']; ver++;
 X.meosArmClockFcFor(doc); await sleep(20);
 const lkR='file:///v.md R_1';
 X._meosChainStart.set(lkR, Date.now()-120e3); ver++; X.meosArmClockFcFor(doc); await sleep(20);
 await X.meosClockStopHere(doc,'R_1',4); await sleep(20);
 ok(/⏸1? v2m ↻5m/.test(A[4]),'  止めると v2m',A[4]);
 await X.meosChainStartHere(doc,'R_1',4); await sleep(20);
 ok(Math.abs((Date.now()-X._meosChainStart.get(lkR))-120e3)<3000,'  v が残っていれば、止めた値(2m)から',Date.now()-X._meosChainStart.get(lkR));
 await X.meosClockStopHere(doc,'R_1',4); await sleep(20);
 A[4]=A[4].replace(/ v\S+/,''); ver++;
 ok(!/ v/.test(A[4]),'  (人が v を消した)',A[4]);
 await X.meosChainStartHere(doc,'R_1',4); await sleep(20);
