...
function renderEditPanelMode(){
  const zooming=editPanelMode==='zoom';const refing=editPanelMode==='reference';/* v0.9.999100 */
  if(editModeSelect)editModeSelect.value=refing?'reference':(zooming?'zoom':'edit');
  if(zoomMePanel)zoomMePanel.classList.toggle('hidden',!zooming);
  const _rp=document.getElementById('ref-create-panel');if(_rp)_rp.classList.toggle('hidden',!refing);
  if(nameWrap)nameWrap.classList.toggle('hidden',zooming||refing || meScope==='all'||meScope==='shadow');
  if(input)input.disabled=zooming||refing || (meScope==='all'||meScope==='shadow');
  if(topButtons)topButtons.classList.toggle('hidden',zooming||refing || meScope==='all'||meScope==='shadow');
  const r=currentMode==='rename';
  if(membranePanel)membranePanel.classList.toggle('hidden',zooming||refing || (!r&&!inMembraneState));
  if(colorRow)colorRow.classList.toggle('hidden',zooming||refing || (!r&&!inMembraneState));
}
...
if(editModeSelect)editModeSelect.addEventListener('change',()=>{const v=editModeSelect.value;/* v0.9.999100: ReferenceはMe Dock内パネルを開く(QuickPick廃止) */editPanelMode=(v==='zoom')?'zoom':(v==='reference')?'reference':'edit';renderEditPanelMode();if(v==='reference'&&typeof window.__refRefreshName==='function')window.__refRefreshName();});/* v0.9.999100: 参照作成パネルの記号選択・カスタム入力・Create */(function(){const rp=document.getElementById('ref-create-panel');if(!rp)return;const symRow=document.getElementById('ref-sym-row'),customRow=document.getElementById('ref-custom-row'),customInput=document.getElementById('ref-custom-input'),nameInput=document.getElementById('ref-name-input'),descInput=document.getElementById('ref-desc-input'),createBtn=document.getElementById('ref-create-btn');window.__refSelFam=window.__refSelFam||1;window.__refSymbols=window.__refSymbols||{};window.__paintRefSyms=function(){const syms=window.__refSymbols||{};rp.querySelectorAll('.ref-sym').forEach(b=>{const f=Number(b.getAttribute('data-fam'));b.classList.toggle('sel',f===window.__refSelFam);if(f>=6&&f<=9)b.textContent=syms[f]||('R'+f);});const cust=window.__refSelFam>=6&&window.__refSelFam<=9;if(customRow)customRow.classList.toggle('hidden',!cust);if(cust&&customInput)customInput.value=syms[window.__refSelFam]||'';};if(symRow)symRow.addEventListener('click',ev=>{const b=ev.target&&ev.target.closest?ev.target.closest('.ref-sym'):null;if(!b)return;window.__refSelFam=Number(b.getAttribute('data-fam'));window.__paintRefSyms();});if(createBtn)createBtn.addEventListener('click',()=>{const fam=window.__refSelFam;const custom=(fam>=6&&fam<=9&&customInput)?(customInput.value||''):'';vscode.postMessage({type:'referenceCreatePanel',fam,name:(nameInput&&nameInput.value)||'参照',desc:(descInput&&descInput.value)||'',custom});editPanelMode='edit';renderEditPanelMode();});/* v0.9.999101(俊克): 名前にタイムスタンプ+↻更新(通常膜と同様)。MeOS形式HHMMSS.月コードDD をwebviewで生成。 */function refStamp(){const d=new Date();const p2=n=>String(n).padStart(2,'0');const mc=['J','F','M','A','5','6','7','8','S','O','N','D'];return p2(d.getHours())+p2(d.getMinutes())+p2(d.getSeconds())+'.'+(mc[d.getMonth()]||'')+p2(d.getDate());}window.__refRefreshName=function(){if(!nameInput)return;const raw=String(nameInput.value||'').trim();const parts=raw.split('_');const isTs=s=>{if(s.length!==10||s.charAt(6)!=='.')return false;for(let i=0;i<6;i++){const c=s.charAt(i);if(c<'0'||c>'9')return false;}for(let i=8;i<10;i++){const c=s.charAt(i);if(c<'0'||c>'9')return false;}return true;};/* v0.9.999102: 正規表現不使用(テンプレートリテラルで\dがd化する罠を回避・俊克バグ1) */if(parts.length>1&&isTs(parts[parts.length-1]))parts.pop();let base=parts.join('_');while(base.length&&base.charAt(base.length-1)==='_')base=base.slice(0,-1);if(!base)base='参照';nameInput.value=base+'_'+refStamp();};const tsBtn=document.getElementById('ref-ts-btn');if(tsBtn)tsBtn.addEventListener('click',()=>{window.__refRefreshName();if(nameInput){nameInput.focus();const v=nameInput.value;const i=v.lastIndexOf('_');/* v0.9.999103(俊克改良2): TS部分のみ選択(全選択は薄く見えにくい) */if(i>=0)nameInput.setSelectionRange(i+1,v.length);else nameInput.select();}});window.__paintRefSyms();})();
...
function closeBmPendingPop(){if(bmPendingPop)bmPendingPop.classList.remove('on');if(typeof closeRefSubmenu==='function')closeRefSubmenu();window.__refDeciding=false;/* v0.9.99988: 閉じたら決定中を解除=次回はtip復活 */}
if(bmPendingMenuBtn)bmPendingMenuBtn.addEventListener('click',ev=>{ev.preventDefault();window.__refDeciding=false;/* v0.9.99988: 開いた直後はtipを出す(操作開始で抑制) */const willOpen=!bmPendingPop.classList.contains('on');bmPendingPop.classList.toggle('on',willOpen);if(!willOpen)return;const r=bmPendingMenuBtn.getBoundingClientRect();requestAnimationFrame(()=>{const h=bmPendingPop.offsetHeight||60,w=bmPendingPop.offsetWidth||150;let left=Math.min(r.right-w,window.innerWidth-w-6);if(left<6)left=6;bmPendingPop.style.left=left+'px';bmPendingPop.style.top=Math.max(6,r.top-h-6)+'px';});});
...
refreshBtn.addEventListener('click',()=>{window.__editTsRefresh=true;/* v0.9.999105: 更新後にTS部分のみ選択(下記modeハンドラで) */vscode.postMessage({type:'refreshTimestamp',mode:currentMode,value:input.value});});
