...
if(editModeSelect)editModeSelect.addEventListener('change',()=>{const v=editModeSelect.value;/* v0.9.999100: ReferenceはMe Dock内パネルを開く(QuickPick廃止) */editPanelMode=(v==='zoom')?'zoom':(v==='reference')?'reference':'edit';renderEditPanelMode();});/* v0.9.999100: 参照作成パネルの記号選択・カスタム入力・Create */(function(){const rp=document.getElementById('ref-create-panel');if(!rp)return;const symRow=document.getElementById('ref-sym-row'),customRow=document.getElementById('ref-custom-row'),customInput=document.getElementById('ref-custom-input'),nameInput=document.getElementById('ref-name-input'),descInput=document.getElementById('ref-desc-input'),createBtn=document.getElementById('ref-create-btn');window.__refSelFam=window.__refSelFam||1;window.__refSymbols=window.__refSymbols||{};window.__paintRefSyms=function(){const syms=window.__refSymbols||{};rp.querySelectorAll('.ref-sym').forEach(b=>{const f=Number(b.getAttribute('data-fam'));b.classList.toggle('sel',f===window.__refSelFam);if(f>=6&&f<=9)b.textContent=syms[f]||('R'+f);});const cust=window.__refSelFam>=6&&window.__refSelFam<=9;if(customRow)customRow.classList.toggle('hidden',!cust);if(cust&&customInput)customInput.value=syms[window.__refSelFam]||'';};if(symRow)symRow.addEventListener('click',ev=>{const b=ev.target&&ev.target.closest?ev.target.closest('.ref-sym'):null;if(!b)return;window.__refSelFam=Number(b.getAttribute('data-fam'));window.__paintRefSyms();});if(createBtn)createBtn.addEventListener('click',()=>{const fam=window.__refSelFam;const custom=(fam>=6&&fam<=9&&customInput)?(customInput.value||''):'';vscode.postMessage({type:'referenceCreatePanel',fam,name:(nameInput&&nameInput.value)||'参照',desc:(descInput&&descInput.value)||'',custom});editPanelMode='edit';renderEditPanelMode();});window.__paintRefSyms();})();
