...
if(refGroupList){const norm=Array.isArray(m.groups)?m.groups:[];window.__refGroups=norm;/* v0.9.99983(俊克バグ1): 参照膜有り/なしを孫メニュー(フライアウト)に=クリックで横にリストを開く */const docs=norm.filter(g=>g.hasMembrane),plains=norm.filter(g=>!g.hasMembrane);const cat=(c,label,arr)=>'<button class="bm-pending-row ref-cat" data-cat="'+c+'" data-tip="クリックで「'+label+'」のグループ一覧を左に開く">'+(arr.some(g=>g.active)?'✓ ':'　')+label+'<span class="ref-arrow">('+arr.length+') ◂</span></button>';refGroupList.innerHTML=norm.length?((docs.length?cat('doc','参照膜有り',docs):'')+(plains.length?cat('plain','参照膜なし',plains):'')):'<div class="bm-pending-row" style="cursor:default">参照グループなし — Edit▾ → Reference で作成</div>';if(typeof closeRefSubmenu==='function')closeRefSubmenu();}
