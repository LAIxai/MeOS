...
console.log('\u247e 既定で ↺↻ / Set は指定してから押せる');
{
  ...
  ok(/dual: _dl0, rounds: _rd0, cycleSrc: _cs0, whenSrc: [^,]*_pMark, manual: _mn0 \}/.test(S11),   /* v4.2.396: 連番を頭に付ける式になった */   /* v4.2.367: Read した行を書き直す引数(atLine)が足された */ '\u2605面が言った同時が、書く所まで届く(v4.2.63: \u25b6\ufe0f も)', true);
  ...
  ok(/function clkTouch\(\)/.test(S11) && /clkDirty=false;[^\n]{0,400}?clkPaintSet\(\)/.test(S11)   /* v4.2.393: 置き場所の旗も開いた時に戻す */,
     '\u2605\u2605開いた時は未設定から始まる', true);
...
console.log('\u247b copy \u23f0 は ⏰行だけを写す');
{
  ...
  ok(/closest\('#clk-lock,#clk-unlock,#clk-anchor,#clk-pring,#clk-rep,#clk-dir,#clk-cyc,#clk-tagin,#clk-copy,#clk-read,#clk-now,#clk-set'\)/.test(S9),
...
console.log('\u3253 秒読みは3段(アーチェリー式)');
{
 ...
 ok(/meosStopRinging\(\); (if \(_cb439\) meosPlayCountBell\(_cb439\); else )?meosPlayWhistle\((_ov436)?\);/.test(_up),   /* v4.2.439: 数え鳴きが始まりの合図の時は笛の代わり */   /* v4.2.436: 🔔を書いた時間はその音で */ '\u2605\u2605\u26050秒= 秒読みを止め、3秒の高音を1つ(黙るのではない)', true);
...
console.log('\u3261 \u23f8\u306f\u3044\u3064\u3082\u8d64 / \u4e00\u89a7\u306e\u5199\u3057\u3092\u65b0\u3057\u304f\u3059\u308b / \u2610\u3092\u62bc\u3057\u3066\u3082\u52b9\u304f');
{
 ...
 ok(/var _hit=\(ev\.target&&ev\.target\.closest\)\?ev\.target\.closest\('#clk-lock,#clk-unlock,#clk-anchor,#clk-pring,#clk-rep,#clk-dir,#clk-cyc,#clk-tagin,#clk-copy,#clk-read,#clk-now,#clk-set'\)/.test(S),
...
console.log('⑬ 入れ子 ((30s 15s)×8 1m)×3 = 並びthatが一段上に居るだけ');
{
 ...
 /* ★★★v4.2.87(俊克「部分で見て同じなら、組み込んでも同じはずでしょ?」)= ×N は直前の1つに掛かる。 */
 ok(JSON.stringify([c('↺↻(8h 5m×2)').cycle,c('↺↻(8h 5m×2)').rounds])===JSON.stringify([c('↺↻(8h (5m)×2)').cycle,c('↺↻(8h (5m)×2)').rounds])
    && c('↺↻(8h 5m×2)').cycle.join()==='8h,5m,5m' && c('↺↻(8h 5m×2)').rounds===null,
    '★★★(8h 5m×2) ＝ (8h (5m)×2) ＝ 8h→5m→5m をずっと', [c('↺↻(8h 5m×2)').cycle,c('↺↻(8h 5m×2)').rounds]);
 ok(c('↺↻5m×2').rounds===2 && c('↺↻5m×2').cycle.join()==='5m' && c('↺15m ×2').rounds===2 && c('↺30s ×1').rounds===1,
    '★★1つだけなら今までどおり回数(5m×2 ＝ (5m)×2 ＝ 2回で終わり)', true);
 ok(c('↻3m 1m×3').cycle.join()==='3m,1m,1m,1m' && c('↻3m 1m×3').rounds===null,
    '★★並びの中の裸の ×N は、全体ではなく直前の1つ', c('↻3m 1m×3').cycle);
 ok(c('↻3m/1mx3').rounds===3 && c('↻3m/1mx3').cycle.join()==='3m,1m',
    '  / の古い形は書かれた時の意味のまま(全体に掛ける)', true);
 ok(c('↺↻(3m 1m)×12').rounds===12 && c('↺↻(8h 5m)×2').rounds===2, '  括弧の後ろの ×N は今までどおり括弧に掛かる', true);
 ...
 {const S=fs.readFileSync(path.join(SRC,'extension.js'),'utf8');
  ...
  ok(/else if\(_cy9&&!String\(_cy9\.value\|\|''\)\.trim\(\)\)clkApplyPreset\(\);/.test(S) && /cyc:'\(\(3m 1m\)\\u00d75\)\\u00d73 \/\/ /.test(S),
...
console.log('⑰ Now / read ⏰ = 0from打ち直させない');
{
 ...
 /* ★★★v4.1.173: webviewの正規表現は**二重エスケープ**で書く(テンプレートリテラルthat単一を食べる)。 */
 ok(/replace\(\/\^\(\\\\d\{4\}\\\\D\\\\d\{1,2\}\\\\D\\\\d\{1,2\}\\\\s\+\\\\d\{1,2\}:\\\\d\{2\}\):\\\\d\{2\}\$\/, '\$1'\)/.test(S),
    '★★★秒を落とすのは「時:分:秒」の形の時だけ(分を巻き込まない)', true);
 /* ★単一エスケープの見張りは check_webview.js(評価後のスクリプトを見る方)に置いた=
    そちらはわざと壊して落ちることを確かめてある。同じ見張りを2つ持たない。 */
...
console.log('㊱ 貼った膜の名前がぶつかったらTSを打ち直す / 🐱▾で全体を修復');
{
 ...
 ok(S2.indexOf("+(sp.ni||1)+'/'+_nn+'-'+(sp.ci||1)")>=0,
    '★★名前の番号/名前の総数-その中の番号(1/9-1)= 名前that変わった所も一目で分かる', true);
...
console.log('㊴ 連なり= 「この膜の時計」は生きている１本を指す');
{
 ...
 ok(/_chain55 = \(meosClockFcScan\(doc\)\.filter\(x => x\.key === c\.key\)\.length >= 2\)/.test(S9),
  ...
  /* ★★★v4.2.77(俊克「起点無し(仮想起点)は、頭にvを付けた方が分かりやすい」)= 本文へ書くのは `v` 付きだけ。
     v4.2.53 の穴(書いた起点が次の周で古い起点になる)は、印で仮想と分かれば消える。動きは check_vstart.js。 */
  ok(/_base53 = _meosChainStart\.get\(lk\) \|\| c\.vAt/.test(S9) && /if \(!\(c\.vAt && Math\.abs\(c\.vAt - _base53\) < 1000\)\) \{/.test(S9),
     '★★★起点を本文に書くなら `v` 付き・値が変わった時だけ(覚え＞本文の v＞控え＞今)', true);
 ...
 {const SA=S9.slice(S9.indexOf("registerCommand('lai-membrane.clockStopAll'"), S9.indexOf("registerCommand('lai-membrane.clockStopAll'")+3000);
  ok(/clearTimeout\(h\); \} catch \(_\) \{ \} _meosPseudoTimers\.delete\(k\)/.test(SA),
     '\u2605\u2605\u2605\u5168\u90e8\u6b62\u3081\u308b\u306f\u300c\u8d77\u304d\u308b\u624b\u300d\u3082\u5916\u3059(\u899a\u3048\u3060\u3051\u6d88\u3059\u3068\u9cf4\u308b)', true);}
 ...
 {let _u=0;
  ...
  ok(JSON.stringify(a.fc)==='[[4,5]]',
     '★★★⏰の行は畳まない/メッセージは畳む(v4.2.52・俊克 バグ1で狭めた)', a.fc);
  ok(JSON.stringify(b.fc)==='[[4,6]]',
     '  待っている1本が増えても、畳むのはメッセージの側だけ', b.fc);
 ...
 {const W=FN(S9,'async function meosAutoFoldSpecLines');
  ok(/if \(MembraneFoldingProvider\._lastSig !== _sigNow48\) \{[\s\S]{0,300}?notifyRangesChanged/.test(W),
     '★★★「範囲が変わった」と言うのは、本当に変わった時だけ(毎回言うとVS Codeが畳みを捨てる)', true);
  ok(/const _sigNow48 = String\(editor\.document\.uri\) \+ '@' \+ editor\.document\.version \+ '\|' \+ Array\.from\(meosFcOutMembranes\(/.test(W) && /MembraneFoldingProvider\._lastSig = String\(document\.uri\) \+ '@' \+ document\.version \+ '\|' \+ Array\.from\(_out\)/.test(S9),
     '★★範囲が変わるのは①本文が変わった②カーソルが塊に出入りした、の2つだけ', true);
  ok(/if \(MembraneFoldingProvider\._lastSig !== _sigNow48\) \{[\s\S]{0,900}?while \(MembraneFoldingProvider\._lastSig !== _sigNow48/.test(W),
     '★言わなかった時は待ちも要らない(取り直しが走らない)/v4.2.425: 待つのは時間でなく Provider の名札', true);
...
console.log('㊵ ⏰のタイトル(v4.2.91 俊克「メッセージ機能を取り込んで、分離したメッセージ機能は廃止」)');
{
 const S49=fs.readFileSync(path.join(SRC,'extension.js'),'utf8');
 const A=['# t','<!-- {* ▼mCN=EYE_1 // 目薬 *} -->','本文','<!-- {* ▲mCN=EYE_1 // *} -->',
  '<!-- Mew!FC mCN (📊⊕0+0D0W) -->',
  '<!-- Mew!UFC ⏰ 1. 2026-09-05 01:00p ↺↻8h // 目薬の時間です -->',
  '<!-- Mew!FC ⏰ 2. ↺↻(5m)×2 // 目薬 1. 本目 -->',
  '<!-- Mew!FC 1. 古いメッセージ -->',''];
 ...
 ok(rows[1].title==='目薬 1. 本目' && rows[1].cycle.join()==='5m' && rows[1].cycleSrc==='(5m)×2',
    '★★★`//` から後ろはタイトル= 時計の中身に混ぜない', [rows[1].title,rows[1].cycle,rows[1].cycleSrc]);
 {const q=X.meosClockFcParse('<!-- Mew!UFC ⏰ ↺↻(5m)×1 // 5分タイマー -->');
  ok(q.cycle.join()==='5m' && q.rounds===1 && q.title==='5分タイマー', '  「5分」の 5 が周期に入らない', [q.cycle,q.title]);
  const q2=X.meosClockFcParse('<!-- Mew!FC ⏰ ↺↻(5m)×1✓ // 5分タイマー -->');
  ok(q2.done===true && q2.title==='5分タイマー', '  ✓ はタイトルの前(済みも読める)', [q2.done,q2.title]);}
 ok(X.meosChainFillSlot('目薬 1. 本目',2)==='目薬 2 本目',
    '★★★タイトルの中の器に回数を入れる', X.meosChainFillSlot('目薬 1. 本目',2));
 ok(X.meosChainSayFor(d,'EYE_1',5,18)==='目薬の時間です' && X.meosChainSayFor(d,'EYE_1',6,2)==='目薬 2 本目',
    '★★★言葉はその1本のタイトルから(並びの外のメッセージFCは数えない)', [X.meosChainSayFor(d,'EYE_1',5,18),X.meosChainSayFor(d,'EYE_1',6,2)]);
 ok(X.meosChainSayFor(d,'EYE_1',999,1)==='',
    '  タイトルが無ければ空= 今までどおり膜の名前を出す', true);
 ok(/const name = _say49 \|\| scope\.name \|\| 'this file';/.test(S49),
    '★★言葉が在れば名前でなく言葉を出す(書かない人には何も変わらない)', true);
 ok(/\(spec\.done \? '\\u2713' : ''\) \+ \(\(spec\.title && String\(spec\.title\)\.trim\(\)\) \? \(' \/\/ ' \+ String\(spec\.title\)\.trim\(\)\) : ''\)/.test(S49)
    && /if \(spec && spec\.title === undefined && typeof atLine === 'number'\)/.test(S49),
    '★★書き戻す口はタイトルを運ばなくても残る(名指しの行から引く)', true);
}
...
console.log('㊷ 走る⏰の並びは畳まない(俊克 pm07:47 改良1)');
{
 ...
 const SH=FN(S51,'function meosFcFoldShape');
  ok(/for \(let ln = Math\.max\(0, head \+ 1\); ln <= end && ln < document\.lineCount; ln\+\+\)/.test(SH),
     '★★★見るのは**畳まれる行だけ**= 全文書を走らない(v4.2.51は再描画のたびに全文書を走り、ホストを5秒止めた)', true);
 ok(/hasRange: \(end > head\) && !_keepOpen51/.test(SH),
    '★★★まだ鳴る⏰が居れば、その並びには畳む範囲を渡さない', true);
  ok(/const c = meosClockFcParse\(t\);\s*\n\s*if \(c && !c\.done\) \{ _keepOpen51 = true; break; \}/.test(SH),
     '★まだ鳴る時計が在る塊だけ開けたまま(済んだ物しか無い塊は今までどおり畳む)', true);
  ok(/if \(t\.indexOf\('\\u23f0'\) < 0\) continue;/.test(SH) && !/meosClockFcScan\(document\)/.test(SH),
